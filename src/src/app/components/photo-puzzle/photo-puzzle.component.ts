...
import { randomRange } from '../../shared/app-utility';

@Component({
  selector: 'app-photo-puzzle',
  templateUrl: './photo-puzzle.component.html',
  styleUrls: ['./photo-puzzle.component.css']
})
export class PhotoPuzzleComponent implements OnInit, AfterViewInit {
  ...
  generateCell(
    ...
    img.src = `../../assets/puzzle-image/${text}.jpg`;
    img.width = 235;
    img.height = 235;
    ...
    cellEle.style.left = `${
      j * (cellWidth + this.cellBorder) + this.outerGap
    }px`;
  ...
  move(cellLocation: CellLocation, moveableDirection: Direction) {
    ...
    if (moveableDirection === 'left' || moveableDirection === 'right') {
      ...
      cell.style.left = `${
        newJ * (cellWidth + this.cellBorder) + this.outerGap
      }px`;
